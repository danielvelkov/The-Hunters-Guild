...
function updatePreview() {
  ...
  const monsterFormDataList = window.monstersForms.map(mf => new FormData(mf[0])).filter(fd => fd.get('monster') != '');
  const selectedMonsterDetailsList = window.selectedMonsters.filter(m => m);
  const bonusRewardsIds = $('#bonus-rewards-enabled').is(':checked') && $('#bonus-rewards').val() ? $('#bonus-rewards').val() : [];

  // Validate before proceeding
  if (monsterFormDataList.length === 0 || monsterFormDataList.length !== selectedMonsterDetailsList.length) {
    $('#quest-preview').html('');
    return;
  }

  // Generate quest title
  const questTitle = generateQuestTitle(questDetails, monsterFormDataList, selectedMonsterDetailsList);
  
  // Check if high rank quest
  const isHighRankQuest = questDetails.get('quest-star-rank') === 'HIGH' || +questDetails.get('quest-star-rank') >= 4;
  ...
  const previewHTML = `
    <div id="tabs">
      ...
      <div id="tabs-quest-details">
        ${generateQuestDetailsTab(questDetails, questTitle, selectedMonsterDetailsList, monsterFormDataList, bonusRewardsIds)}
      </div>
      ${selectedMonsterDetailsList.map((m, i) => `
        <div id="tabs-monster-${i}">
          ${generateMonsterTab(m, isHighRankQuest, monsterFormDataList[i], bonusRewardsIds)}
        </div>
      `).join('')}
    </div>
  `;
...
function generateQuestTitle(questDetails, monsterFormDataList, selectedMonsterDetailsList) {
  let questTitle = `${questDetails.get('quest-type')} `;
  
  if (selectedMonsterDetailsList.length === 1) {
    const monsterName = formatMonsterName(monsterFormDataList[0], selectedMonsterDetailsList[0]);
...
function formatMonsterName(monsterFormData, monster) {
  return `${monsterFormData.get('tempered') === 'on' ? 'Tempered ' : ''}${monsterFormData.get('frenzied') === 'on' ? 'Frenzied ' : ''}${monsterFormData.get('arch-tempered') === 'on' ? 'Arch-Tempered ' : ''}${monster.name}`.trim();
}

function generateQuestDetailsTab(questDetails, questTitle, selectedMonsterDetailsList, monsterFormDataList, bonusRewardsIds) {
  return `
    <table class="themetable" width="100%" align="center">
      <tbody>
        <tr>
          ...
          <th colspan="3" align="right">${questDetails.get('quest-star-rank')}${questDetails.get('quest-star-rank') > 1 ?
            [...new Array(+questDetails.get('quest-star-rank'))].map(star => '⭐').join('') : '⭐'}</th>
        </tr>
        <tr>
...
function generateBonusRewards(bonusRewardsIds) {
  ...
  return bonusRewardsIds.map(id => {
    const bonusItem = window.bonusQuestRewardsList.find(qr => qr.id === id);
    if (!bonusItem) return '';
    
    const iconPath = `url('../icons/Item Icons/${bonusItem.icon === 'INVALID' ? 'ITEM_0001' : bonusItem.icon}.png')`;
    return `<div class="item-img-container bonus-item" title="${bonusItem.name}\nRarity: ${bonusItem.rarity}"
        data-item-id="${bonusItem.id}"
        style="--item-color: var(--${bonusItem.iconColor}); --item-icon:${iconPath}; ">
        <img height='30' src="icons/Item Icons/${bonusItem.icon === 'INVALID' ? 'ITEM_0001' : bonusItem.icon}.png" />
        ${(+bonusItem.rarity >= 6 && bonusItem.type === 'Rare Drop') || (bonusItem.type === 'Food Ingredient' && +bonusItem.rarity >= 5) ?
        `<div class="sparkles">${[...Array(6)].map(m => `<div class="sparkle"></div>`).join('')}</div>` : ''}
      </div>`;
  }).join('');
}
...
function generateRecommendedSkills(selectedMonsterDetailsList) {
  return selectedMonsterDetailsList
    ...
    .map(s => {
      ...
      skillDiv.append(`<img src="icons/Skill Icons/${s.icon}.png" height="23" alt="Skill_Icon"/> <span>${s.name}</span>`);
...
function generateMonsterTab(monster, isHighRankQuest, monsterFormData, bonusRewardsIds) {
  return `
    <h2>${monster.name}</h2>
  ...
  return `
    <table style="width:fit-content;" class="dmg-effectiveness-table">
      <tbody align="center">
        ...
        ${monster.part_dmg_effectiveness.map(de => {
          let tableRow = '<tr>';
          tableRow = tableRow.concat(`<td align="left"><img height="20" src="icons/Item Icons/${de.icon}.png"/> <b>${de.name}</b></td>`);
          de.damages.forEach(d => {
            let dmgColor = 'white';
            switch(d.type){
              case 'fire': dmgColor = 'red';
                break;
              case 'water': dmgColor = 'blue';
                break;
              case 'thunder': dmgColor = 'yellow';
                break;
              case 'ice': dmgColor = 'cyan';
                break;
              case 'dragon': dmgColor = 'purple';
                break;
            }
            tableRow = tableRow.concat(`<td style="background-color: ${getDmgColor(d.value, dmgColor)}">${d.value > 45 || (dmgColor !== 'white' && d.value >= 20) ?
              `<b ${d.value >= 25 && dmgColor !== 'white' && dmgColor !== 'yellow' && dmgColor !== 'cyan' ? 'style="color:white"' : ''} >`+d.value+'</b>' : d.value}</td>`)
          });
...
function generateStatusEffectivenessTable(monster) {
  return `
    <table class="dmg-effectiveness-table">
      ...
      <tbody>
        ${monster.status_effectiveness.map(se => {
          let tableRow = $('<tr>');
          tableRow.append(`<td align="left"><img height="20" src="icons/Status Icons/${se.icon}.png"/> <b>${se.name}</b></td>`);
...
function generateItemEffectivenessTable(monster) {
  return `
    <table class="dmg-effectiveness-table">
      ...
      <tbody>
        ${monster.item_effectiveness.map(ie => {
          let tableRow = $('<tr>');
...
function createMonsterDropsSection(monster, rank, isTempered, bonusRewardsIds) {
  ...
  const bonusRewards = bonusRewardsIds.map(id => window.bonusQuestRewardsList.find(qr => qr.id === id));

  if (!monsterDrops[rank]) {
    dropsSection.append(`<p>No drop data is available for this monster at quest rank: <i>(${rank})</i></p>`);
  ...
  Object.keys(monsterDrops[rank]).forEach(rewardType => {
    const containsBreakablePartsCol = monsterDrops[rank][rewardType].some(i => i.brokenPart);
    ...
    if (containsBreakablePartsCol)
      colRow.append('<th>Part</th>');
      ...
      dataRow.append(`
        <td title="${'Rarity: ' + p.rarity + '\n' + p.description}">
          <div style="display:flex; gap:0.5em; align-items:center;">
            <div class="item-img-container"
              data-icon-id="${p.icon}"
              style="--item-icon:${itemIconPath}; --item-color: var(--${p.iconColor});">
    ...
    const rewardConditions = {
      'tempered': () => !isTempered,
      'basic rewards': () => !bonusRewards.some(r => r && r.name === 'Basic Material'),
      'valuable rewards': () => !bonusRewards.some(r => r && r.name === 'Valuable Material'),
      'rare rewards': () => !bonusRewards.some(r => r && partsArr.find(pr => pr.name === r.name))
    };
    ...
    if (rewardType.toLowerCase().includes('carv'))
    ...
    else if (rewardType.toLowerCase().includes('wound') ||
      ...
      similarSections.bonus.append(dropTable);
  });
...
function groupMonsterDrops(monster) {
  const monsterDrops = window.monstersDropsList.filter(md => md.source === monster);
  const monsterDropsGroup = Object.groupBy(monsterDrops, ({rank}) => rank);
  
  Object.keys(monsterDropsGroup).forEach((k) => {
    const rewardGroups = Object.groupBy(monsterDropsGroup[k], ({rewardType}) => rewardType);
    Object.keys(rewardGroups).forEach((rewardKey) => {
      rewardGroups[rewardKey] = rewardGroups[rewardKey].sort((a,b) => b.probability - a.probability);
    });
